...
watch(isLoaded, (val) => {
  if (val) {
    ...
    lights.forEach(light => {
      ...
      light.intensity = 0
      gsap.to(light, { intensity: targetIntensity, duration: 2, ease: "power2.in" })
    })
  }
})
...
watch(() => route.path, (newPath) => {
  ...
  if (newPath === '/about') {
    gsap.to(model.rotation, { x: 0.5, y: Math.PI, z: 0.2, duration: 1.5, ease: "power2.inOut" })
    gsap.to(camera.position, { x: -2, z: 6, duration: 2 })
  } else if (newPath === '/work') {
    gsap.to(model.rotation, { x: 0, y: Math.PI * 0.5, z: 0, duration: 1.5, ease: "power2.inOut" })
    gsap.to(camera.position, { x: 2, z: 5, duration: 2 })
  } else if (newPath === '/contact') {
    gsap.to(model.rotation, { x: -0.3, y: 0, z: -0.3, duration: 1.5, ease: "power2.inOut" })
    gsap.to(camera.position, { x: 0, z: 8, duration: 2 })
  } else {
    gsap.to(model.rotation, { x: 0, y: 0, z: 0, duration: 1.5, ease: "power2.inOut" })
